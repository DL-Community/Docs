(function () {
    ...
    var lastCompletedDocumentPath = normalizeRoute(currentPath());
    ...
    function resetNewDocumentScrollPosition() {
        var nextPath = normalizeRoute(currentPath());
        var pathChanged = nextPath !== lastCompletedDocumentPath;
        lastCompletedDocumentPath = nextPath;
        if (!pathChanged || currentHeadingId()) return;

        window.requestAnimationFrame(function () {
            if (normalizeRoute(currentPath()) !== nextPath || currentHeadingId()) return;
            window.scrollTo(0, 0);
        });
    }
    ...
    var docsifyCoreMobileMedia = '(max-width: 640px)';
    ...
    function resetDocsifyV5MobileSidebarState(toggle, sidebar) {
        ...
        if (innerToggle) {
            ...
            innerToggle.setAttribute('aria-label', uiText('sidebar_show'));
        }
    }
    ...
    function syncSidebarToggleState() {
        ...
        var mobile = window.matchMedia(docsMobileLayoutMedia).matches;
        var sidebar = document.querySelector('.sidebar');
        ...
        if (usesV5State) {
            ...
            document.body.classList.toggle('close', mobile ? expanded : !expanded);
        } else {
            expanded = mobile
    ...
    function setupSidebarToggle() {
        ...
        var mobileBackdrop = document.querySelector('.mobile-sidebar-shadow');
        var mobile = window.matchMedia(docsMobileLayoutMedia).matches;
        var compactDesktop = !mobile && window.matchMedia(docsCompactDesktopMedia).matches;
        ...
        if (
            ...
            toggle.dataset.docsV5MobileInitialized !== 'true'
        ) {
            toggle.dataset.docsV5MobileInitialized = 'true';
            resetDocsifyV5MobileSidebarState(toggle, sidebar);
        }
        if (!toggle.dataset.docsHeaderToggle) {
            toggle.dataset.docsHeaderToggle = 'true';
            var wasMobile = mobile;
            var wasCompactDesktop = compactDesktop;
            var compactDesktopQuery = window.matchMedia(docsCompactDesktopMedia);
            var docsifyCoreMobileQuery = window.matchMedia(docsifyCoreMobileMedia);
            toggle.addEventListener('click', function () {
                window.setTimeout(syncSidebarToggleState, 0);
            });
            docsifyCoreMobileQuery.addEventListener('change', function (event) {
                if (
                    !event.matches &&
                    window.matchMedia(docsMobileLayoutMedia).matches
                ) {
                    // Docsify restores its desktop sidebar immediately above
                    // 640px. Keep it closed until the site layout also leaves
                    // its wider mobile breakpoint.
                    resetDocsifyV5MobileSidebarState(toggle, sidebar);
                    syncSidebarToggleState();
                }
            });
            window.addEventListener('resize', function () {
                var mobileNow = window.matchMedia(docsMobileLayoutMedia).matches;
                var compactDesktopNow = !mobileNow && compactDesktopQuery.matches;
                var layoutTierChanged =
                    mobileNow !== wasMobile ||
                    compactDesktopNow !== wasCompactDesktop;
                if (layoutTierChanged) {
                ...
                if (
                    ...
                    resetDocsifyV5MobileSidebarState(
                        toggle,
                ...
                if (
                    !mobileNow &&
                    !compactDesktopNow &&
                    (wasMobile || wasCompactDesktop) &&
                    usesDocsifyV5SidebarState(sidebar) &&
                    !sidebar.classList.contains('show')
                ) {
                    // Restore the wide desktop sidebar through Docsify's native
                    // state only after leaving both constrained layouts.
                    toggle.click();
                }
                wasMobile = mobileNow;
                wasCompactDesktop = compactDesktopNow;
                syncSidebarToggleState();
                setupPageOutlinePanel();
                if (layoutTierChanged) {
                    // Commit the final off-screen/on-screen position while
                    // positional transitions are disabled. Otherwise restoring
                    // the transition can replay the breakpoint correction.
        ...
        if (mobileBackdrop && mobileBackdrop.dataset.docsSidebarBackdropBound !== 'true') {
            ...
            mobileBackdrop.addEventListener('click', function (event) {
                ...
                if (!window.matchMedia(docsCompactDesktopMedia).matches) return;
                var expanded = usesDocsifyV5SidebarState(sidebar)
                    ? Boolean(sidebar && sidebar.classList.contains('show'))
                    : document.body.classList.contains('close');
                if (!expanded) return;
                toggle.click();
                window.setTimeout(syncSidebarToggleState, 0);
        ...
        if (
            ...
            new MutationObserver(function () {
                // Docsify may update `.sidebar.show` after its link handler
                // runs (notably for in-page outline links). Follow the source
                // state itself so the backdrop, scroll lock, and toggle never
                // retain the previous drawer state.
                syncSidebarToggleState();
            }).observe(sidebar, {
                attributes: true,
            ...
            sidebar.addEventListener('click', function (event) {
                ...
                if (link) {
                    if (
                        window.matchMedia(docsMobileLayoutMedia).matches &&
                        !window.matchMedia(docsifyCoreMobileMedia).matches &&
                        sidebar.classList.contains('show') &&
                        !navigationLinkIsExternal(link)
                    ) {
                        // Let the link update the route before releasing the
                        // fixed-body scroll lock. Closing synchronously can
                        // restore the previous page offset over Docsify's
                        // `auto2top` navigation and can interrupt hash routing.
    ...
    window.$docsify.plugins = [
        function (hook) {
            ...
            hook.doneEach(function () {
                ...
                setupMobileUtilities();
                setupSearchUi();
                resetNewDocumentScrollPosition();
                finishRouteRender();
            });
        }
