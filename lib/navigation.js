(function () {
    ...
    function setTheme(theme, persist) {
        ...
        if (button) {
            button.setAttribute(
                'aria-label',
                theme === 'dark'
                    ? uiText('theme_to_light')
                    : uiText('theme_to_dark')
            );
    ...
    function renderCategoryPage(listItem) {
        ...
        var breadcrumb = createBreadcrumb(categoryTrail(listItem));
    ...
    function renderSectionLandingPage() {
        ...
        var navigationLink = Array.prototype.find.call(
        ...
        var links = Array.prototype.map.call(rootList.children, function (listItem) {
        ...
        var breadcrumb = createBreadcrumb([
            {
                label: uiText('site_title'),
                href: '#' + localizedPath('/about/home', languageDefinitionForPath(currentPath()).code)
            },
            { label: title }
        ]);
        if (breadcrumb) content.appendChild(breadcrumb);
    ...
    function setupBreadcrumb() {
        ...
        var pageLink = currentPageLink();
        if (!heading || !pageLink) return;

        var oldBreadcrumb = content.querySelector('.docs-breadcrumb');
        if (oldBreadcrumb) oldBreadcrumb.remove();

        var trail = [];
        var listItem = pageLink.parentElement;
        while (listItem && listItem.closest('.sidebar-nav')) {
            var link = directChild(listItem, 'a:not(.section-link)');
            if (link) {
                trail.unshift({
