(function () {
    ...
    function categoryBasePath(listItem) {
        var parentList = listItem && listItem.parentElement;
        var ancestor = parentList && parentList.closest('li');
        while (ancestor) {
            var ancestorLink = directChild(
                ancestor,
                'a:not(.section-link):not(.category-page-link)'
            );
            if (ancestorLink && sectionFromHref(ancestorLink.getAttribute('href')) === sectionFromPath(currentPath())) {
                return (ancestorLink.getAttribute('href') || '')
                    .replace(/^#/, '')
                    .split('?')[0];
            }
            parentList = ancestor.parentElement;
            ancestor = parentList && parentList.closest('li');
        }

        return sectionLandingPath(
            sectionFromPath(currentPath()),
            languageDefinitionForPath(currentPath()).code
        );
    }
    ...
    function sectionLabel(section, navigationLink) {
        var key = 'section_' + section;
        var label = uiText(key);
        return label === key && navigationLink ? navigationLink.textContent.trim() : label;
    }
    ...
    function documentBreadcrumbTrail(documentTrail, sidebarNav) {
        ...
        if (hasSectionLanding) {
            trail.push({
                label: sectionLabel(section, navigationLink),
    ...
    function createCategoryCard(link) {
        ...
        if (link.classList.contains('category-page-link')) {
    ...
    function renderSectionLandingPage() {
        ...
        var title = sectionLabel(section, navigationLink);
    ...
    function setupCategoryPages() {
        ...
        candidates.forEach(function (listItem, index) {
            ...
            link.href = '#' + categoryBasePath(listItem) + '?category=' + encodeURIComponent(key);
    ...
    function schedulePageOutlineRefresh() {
    ...
    function exitCategoryToSameRoute(event) {
        if (event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey ||
            event.shiftKey || event.altKey || !currentCategoryKey()) return;

        var link = event.target.closest && event.target.closest('a[href]');
        if (!link) return;
        var targetHash = (link.getAttribute('href') || '').replace(/^#/, '');
        var targetQuery = targetHash.split('?')[1] || '';
        if (!targetHash || new URLSearchParams(targetQuery).has('category') ||
            normalizeRoute(targetHash) !== normalizeRoute(currentPath())) return;
    ...
    document.addEventListener('click', exitCategoryToSameRoute);
