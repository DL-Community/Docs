(function () {
    ...
    function replaceLegacyCallout(line, lineBreak) {
        var match = line.match(/^( {0,3})((?:>[ \t]*)*)([!?])>[ \t]?([^\r\n]*)(\r?)$/);
        if (!match) return line;

        var prefix = match[1] + match[2];
        var type = match[3] === '?' ? 'NOTE' : 'CAUTION';
        var content = match[4];
        var carriageReturn = match[5];
        var continuationBreak = carriageReturn ? '\n' : lineBreak;

        return prefix + '> [!' + type + ']' + carriageReturn
            + (content ? continuationBreak + prefix + '> ' + content + carriageReturn : '');
    }
    ...
    function insertPlaceholders(markdown) {
        ...
        return lines.map(function (line) {
            ...
            return replaceInlineMarkers(replaceLegacyCallout(line, lineBreak));
        }).join('\n');
    }
